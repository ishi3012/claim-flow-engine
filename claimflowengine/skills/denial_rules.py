...
def rule_medical_necessity(claim: pd.Series) -> pd.Series:
    ...
    Parameters:
        claim (pd.Series): A single claim claim
    """
    if claim["icd10_code"].startswith("R") or (
        80000 <= int(claim["cpt_code"]) <= 89999 and claim["patient_age"] < 18
    ):
        return Denial(
