...
WEIGHTS: Dict[str, float] = config.get("model_selection", {}).get("weights", {"auc": 0.33, "f1": 0.33, "recall": 0.34})
...
def composite_score(auc:float, f1: float, recall:float, weights: Dict[str, float]) -> float:
    """
...
def evaluate_models(models: Dict[str, Any], X: pd.DataFrame, y: pd.Series) -> List[Dict[str, Any]]:
    """
    ...
    for name, model in models.items():
        ...
        score = composite_score(np.mean(auc_scores), np.mean(f1_scores), recall, WEIGHTS)
        ...
        print(f"{name.upper()} | AUC: {np.mean(auc_scores):.4f} | F1: {np.mean(f1_scores):.4f} | Recall: {recall:.4f} | Composite: {score:.4f}")
    
    return results

def save_best_model(models: Dict[str, Any], results:List[Dict[str, Any]], X:pd.DataFrame, y:pd.Series, save_dir:Path) -> None:
    """
